...
def container_computation(
    ...
    Args:
        ...
        methods (List[str]): How to compute latency target, possible values: `erms`, `rhythm`, `grandSLAm`
    ...
    for var_index, (sla, workload) in enumerate(slas_workloads):
        ...
        for method in methods:
            if method == "erms":
                ...
                calculator = ContributionBased(data_path, method)
                if prio is None or not prio:
                    calculator.non_priority_main(
                        cpu_inter, mem_inter, workload_dict, sla_in_us, services
                    )
                else:
            ...
            method_results[method] = (
                latency_targets.assign(**extra_cols),
                containers.assign(**extra_cols),
            )
        # Limits rhythm/grandSLAm/firm's containers
        if "erms" in methods:
            latency_target_erms, container_erms = method_results["erms"]
            latency_target_result.append(latency_target_erms)
            container_result.append(container_erms)
            for method in methods:
                latency_target_others, container_others = method_results[method]
                if method == "erms":
                    continue
                for idx, row in latency_target_others.iterrows():
                    latency_target_others.at[idx, "container"] = min(
                        2
                        * latency_target_erms.loc[
                            latency_target_erms["microservice"] == row["microservice"],
                            "container",
                        ].iloc[0],
                        row["container"],
                    )
                latency_target_result.append(latency_target_others)
                for idx, row in container_others.iterrows():
                    container_others.at[idx, "container"] = min(
                        2
                        * container_erms.loc[
                            container_erms["microservice"] == row["microservice"],
                            "container",
                        ].iloc[0],
                        row["container"],
                    )
                container_result.append(container_others)
        else:
            for method in methods:
                latency_targets, containers = method_results[method]
                latency_target_result.append(latency_targets)
                container_result.append(containers)
...
CONFIG = {
    ...
    "scripts": {
        ...
        # "hotel-reserv": {
        #     "Search": "/scripts/hotel-reservation/search.lua",
        #     "Recommendation": "/scripts/hotel-reservation/recommendation.lua",
        #     "Login": "/scripts/hotel-reservation/login.lua",
        # },
        "hotel-reserv": {
    ...
    "interference": [
        {
            "cpu": {
                ...
                "allocation": {
                    ...
                    "izj6c6vb9bfm8mxnvb4n45z": {"idle": 5, "busy": 0},
                    "izj6c6vb9bfm8mxnvb4n44z": {"idle": 5, "busy": 0},
                },
            },
            "mem": {
                "cpu_size": 0.01,
...
def get_ip(container):
    return {"wrk1": "10.244.2.3", "wrk2": "10.244.6.111"}[container]
