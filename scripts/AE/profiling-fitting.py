...
if __name__ == "__main__":
    ...
    output_path = f"{args.profiling_data}/data_{args.app}"
    ...
    fitter.erms_main()
    fitter.usage_fitting()

    erms_result = pd.read_csv(f"{output_path}/fittingResult/ermsFull.csv")
    erms_accy = erms_result.loc[erms_result["accy"] > 0.5]["accy"].mean()
    print("Fitting Success!")
    print(f"The average accuracy of all models are {erms_accy}")
