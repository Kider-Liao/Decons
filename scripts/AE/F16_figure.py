...
parser = argparse.ArgumentParser(
    description=(
        "This script will generate the Figure 16 in the paper. To generate the figure, "
        "you need to generate data first. Please check the script F16_data.sh for more detail. "
    ),
...
files = [x for x in os.listdir(f"{directory}/figure_16") if x[-4:] == ".csv"]
df = []
for file in files:
    df.append(pd.read_csv(f"{directory}/figure_16/{file}"))
df = pd.concat(df)
...
raw, bins = np.histogram(Erms["prio"], bins, density=True)
cdf = np.cumsum(raw)
...
fig.tight_layout()
plt.savefig(f"{directory}/figure_16/figure_16.png")
print(f"Figure is saved in {directory}/figure_16/figure_16.png")
