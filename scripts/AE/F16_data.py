...
parser = argparse.ArgumentParser(
    description=(
        ...
        "with and without priority scheduling use Erms, GrandSLAm and Rhythm for Ali tracing. "
        "To generate the corresponding figure (Figure 16 in the paper), "
        "please run F16_figure.sh. "
        "Ususally, this script will consume 15 minutes."
    ),
...
os.system(f"mkdir -p {output_path}/figure_16")
data_path = "AE/data_ali"
slas_workloads = [
    ({"A": a / 1000, "B": b / 1000}, {"A": c * 300, "B": d * 300})
    for a in [250, 310]
    for b in [1000, 1600]
    for c in range(1, 21)
    for d in range(1, 21)
