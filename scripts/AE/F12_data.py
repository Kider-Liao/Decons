...
parser = argparse.ArgumentParser(
    description=(
        ...
        "methods. Noticed that, the resource allocation result of this script is shown in figure_11, "
        "please check F11_data.sh for more details."
        "To generate the corresponding figure (Figure 12 in the paper), "
        "please run F12_figure.sh. "
...
parser.add_argument(
    ...
    "-a",
    dest="app",
    help="Application name. (`hotel-reserv`, `media-microsvc`)",
...
os.system(f"mv {data_path}/figure_12 {data_path}/figure_12_original")
os.system(f"mkdir -p {data_path}/figure_12")
os.system(f"mkdir -p {output_path}/figure_12")
sharing_main(
...
os.system(f"mv {data_path}/figure_12/* {output_path}/figure_12")
os.system(f"rmdir {data_path}/figure_12")
os.system(f"mv {data_path}/figure_12_original {data_path}/figure_12")

result = statistics(f"{output_path}/figure_12/{'_'.join(CONFIG['services'][app])}")
result.to_csv(f"{output_path}/figure_12/{app}.csv", index=False)
